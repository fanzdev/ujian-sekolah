  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const c = sanitizeLogoUrl(b.logo_url)
        if (c) setLogo(resolveLogoUrl(c))
        else if (b.logo_url && (b.logo_url.includes('vcbt') || b.logo_url.includes('logo.svg') || b.logo_url.includes('favicon.svg'))) { try { const p=JSON.parse(raw) as Record<string,unknown>; (p as Record<string,unknown>)['logo_url']=null; localStorage.setItem('cbt-branding', JSON.stringify(p)) } catch { void 0 } }
      }
    } catch (_e) { void _e }
    import('@/services/settings.service').then(({ fetchSchoolSettings }) =>
      fetchSchoolSettings().then((s) => { const c=sanitizeLogoUrl(s.logo_url); if (c) setLogo(resolveLogoUrl(c)) }).catch(() => undefined),
    )
  }, [])
  return (
