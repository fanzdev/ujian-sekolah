...
export function SplashScreen({ visible }: { visible: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute -bottom-44 -left-44 h-[640px] w-[640px] rounded-full bg-sky-500/10 blur-3xl dark:bg-sky-500/[0.06] will-change-transform" style={{ transform: 'translateZ(0)', animation: 'ssFloat 9s ease-in-out infinite reverse' }} />
      ...
      >
        <div className="relative" style={{ animation: 'ssLogo 2.2s ease-in-out infinite' }}>
          <div className="absolute inset-0 -z-10 scale-[1.75] rounded-[2rem] bg-primary-500/14 blur-[30px] dark:bg-primary-500/20 will-change-transform" style={{ transform: 'translateZ(0)' }} />
          <div
          ...
          >
            <img src={`${import.meta.env.BASE_URL}logo.svg`} alt="Logo SMK AL-FATA" className="h-[60px] w-[60px] object-contain" width={60} height={60} loading="eager" decoding="sync" />
          </div>
          <span className="absolute -bottom-1 -right-1 flex h-7 w-7 items-center justify-center rounded-full bg-primary-600 text-white shadow-lg ring-4 ring-white dark:ring-slate-950" style={{ animation: 'ssBadge 2s ease-in-out infinite' }}>
        ...
        <h1 className="mt-7 bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-[22px] font-extrabold tracking-tight text-transparent dark:from-white dark:to-slate-300" style={{ letterSpacing: '-0.02em', animation: 'ssText 0.6s ease-out 0.2s both' }}>SMK AL-FATA CBT</h1>
        <p className="mt-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-primary-600 dark:text-primary-300" style={{ animation: 'ssText 0.6s ease-out 0.3s both' }}>Computer Based Test</p>
        ...
        <div className="mt-8 flex items-center gap-2.5" aria-label="Memuat" style={{ animation: 'ssText 0.5s ease-out 0.5s both' }}>
          <span className="h-2 w-2 rounded-full bg-primary-500 shadow-sm shadow-primary-500/30 will-change-transform" style={{ animation: 'ssDotPro 1.1s cubic-bezier(0.4,0,0.2,1) infinite' }} />
          <span className="h-2 w-2 rounded-full bg-primary-500 shadow-sm shadow-primary-500/30 will-change-transform" style={{ animation: 'ssDotPro 1.1s cubic-bezier(0.4,0,0.2,1) 0.16s infinite' }} />
          <span className="h-2 w-2 rounded-full bg-primary-500 shadow-sm shadow-primary-500/30 will-change-transform" style={{ animation: 'ssDotPro 1.1s cubic-bezier(0.4,0,0.2,1) 0.32s infinite' }} />
        </div>
        ...
        <div className="relative mt-5 h-[6px] w-48 overflow-hidden rounded-full bg-slate-200/80 p-[2px] shadow-inner dark:bg-slate-800" style={{ animation: 'ssText 0.5s ease-out 0.55s both' }}>
          <div className="h-full w-full overflow-hidden rounded-full">
            <div
              className="h-full w-[44%] rounded-full bg-gradient-to-r from-primary-500 via-sky-500 to-primary-600 will-change-transform"
              style={{ transform: 'translateZ(0)', animation: 'ssBarPro 1.15s cubic-bezier(0.4,0,0.6,1) infinite alternate', boxShadow: '0 0 10px rgba(59,130,246,0.4)' }}
            />
          </div>
      ...
      <div className="absolute bottom-5 flex flex-col items-center gap-1.5 px-4 text-center" style={{ transition: 'opacity 560ms ease 200ms', opacity: visible ? 1 : 0 }}>
        <p className="text-[10px] font-medium uppercase tracking-[0.14em] text-slate-400 dark:text-slate-600">SMK AL-FATA • v1.0 • Profesional • Aman • Cepat</p>
        <p className="text-[11px] text-slate-400 dark:text-slate-500">© 2026 SMK AL-FATA CBT</p>
      </div>
