    ...
    <>
      <div className="hidden overflow-x-auto scrollbar-thin sm:block">
        <table className="w-full min-w-[640px] border-collapse">
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50/80 dark:bg-slate-800 dark:text-slate-200">
              {columns.map((col) => (
    ...
    <div className="flex flex-col gap-3 border-t border-slate-100 px-3 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:px-4">
      ...
      <nav className="flex items-center justify-center gap-1 sm:justify-end" aria-label="Navigasi halaman">
        <button
          ...
          className="tap-target rounded-xl px-3 py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-100 disabled:opacity-40 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:bg-slate-700 dark:bg-slate-700 dark:text-slate-200"
        >
          ‹ Prev
        </button>
        {pages.map((p, idx) =>
