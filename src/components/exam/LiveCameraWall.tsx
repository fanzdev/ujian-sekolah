...
import { Camera, RefreshCw, Clock, User, VideoOff, Radio, Image as ImageIcon } from 'lucide-react'
import { supabase } from '@/services/client'
import { Card, CardBody } from '@/components/ui/Card'
import { Badge } from '@/components/ui/Badge'
import { Button } from '@/components/ui/Button'
import { relativeTime } from '@/lib/datetime'
import { AdminLiveGrid } from '@/components/exam/LiveCameraRTC'
    ...
    <Card className="mt-5 overflow-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 dark:border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800 dark:text-slate-100">Kamera Live</p>
            <p className="text-[11px] text-slate-400">Real-time WebRTC bila tersedia, fallback snapshot JPG. Refresh 15s.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-full bg-slate-100 p-1 dark:bg-slate-800">
            <button onClick={() => setMode('live')} className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold transition ${mode === 'live' ? 'bg-white text-primary-600 shadow dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
              <Radio className="h-3.5 w-3.5" /> Live
            </button>
            <button onClick={() => setMode('snapshot')} className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold transition ${mode === 'snapshot' ? 'bg-white text-primary-600 shadow dark:bg-slate-700 dark:text-white' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
      ...
      <CardBody>
          ...
          <>
            {loading && snapshots.length === 0 ? (
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {snapshots.map((s, idx) => (
                  <div key={`${s.url}-${idx}`} className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md dark:border-slate-700 dark:bg-slate-800">
                    <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                      <img src={s.url} alt={`Snapshot ${s.ownerName}`} className="h-full w-full object-cover transition group-hover:scale-[1.02] bg-white dark:bg-slate-700" loading="lazy" onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
                      <div className="pointer-events-none absolute inset-0 hidden items-center justify-center bg-slate-100 text-slate-400 group-[&:has(img[style*='display: none'])]:flex dark:bg-slate-700 dark:text-slate-500">
                      ...
                      <span className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-white backdrop-blur">
                        <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /> SNAPSHOT
                      </span>
        ...
        {mode === 'snapshot' && (
          <p className="mt-4 text-[11px] leading-relaxed text-slate-400">
            Mode Snapshot: menampilkan <strong>JPG berkala</strong> setiap ~2 menit. Untuk video real-time pilih tab <strong>Live</strong> di atas — WebRTC peer-to-peer via Supabase Realtime (butuh izin kamera siswa). Setiap snapshot ~15-30KB di bucket <span className="font-mono">media/exam-snapshot/</span>.
          </p>
        )}
        {mode === 'live' && (
          <p className="mt-4 text-[11px] leading-relaxed text-slate-400">
            Mode Live: <strong>WebRTC peer-to-peer</strong> langsung dari kamera siswa ke browser admin via Supabase Realtime signaling (STUN <span className="font-mono">stun.l.google.com:19302</span>). Tidak menyimpan video di storage, hanya snapshot fallback jika admin pilih tab Snapshot.
          </p>
        )}
      </CardBody>
    </Card>
