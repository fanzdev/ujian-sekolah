...
import { listClasses, listSubjects } from '@/services/academics.service'
...
type ImportKind = 'students' | 'teachers' | 'subjects' | 'question_banks' | 'exams' | 'grades'
...
const KIND_ICONS: Record<ImportKind, React.ReactNode> = {
  ...
  subjects: <Database className="h-4 w-4" />,
...
function ImportPanel() {
  ...
  const kindSchema = useMemo(() => {
    switch (kind) {
      ...
      case 'subjects': return subjectRowSchema
  ...
  const subjectByCode = useMemo(() => new Map((subjectsQuery.data ?? []).map((s) => [norm(s.code), s.id])), [subjectsQuery.data])
    ...
    for (const row of validRows) {
      try {
        ...
        if (kind === 'students') {
          ...
          const { error } = await supabase.from('subjects').insert({ code, name, description })
          ...
          if (subjectCode) {
            const sid = subjectByCode.get(norm(subjectCode))
            if (!sid) throw new Error(`Kode mapel "${subjectCode}" tidak ditemukan`)
            subjectId = sid
          }
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-4 dark:border-slate-700 dark:bg-slate-800/30">
              ...
              {kind === 'students' && (
                <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs leading-relaxed text-amber-800 dark:bg-amber-500/10 dark:text-amber-200">
                  ...
                  {kind === 'students' && (subjectsQuery.data ?? []).length === 0 && null}
                </p>
              )}
              {kind === 'exams' && (
        ...
        <Card className="border-primary-100 bg-primary-50/40 dark:border-primary-800/40 dark:bg-primary-500/5">
          <CardBody>
            ...
            <ul className="mt-3 list-disc space-y-1.5 pl-4 text-xs leading-relaxed text-primary-900/70 dark:text-primary-200/70">
              ...
              {kind === 'subjects' && (
                <>
                  <li>Kode unik (huruf/angka/_/-), max 20</li>
                  <li>Nama wajib 2-100 karakter</li>
                  <li>Kode duplikat akan gagal (sudah ada di DB)</li>
                </>
              )}
              {kind === 'question_banks' && (
                <>
                  ...
                  <li>Bank tujuan wajib dipilih — mapel otomatis ikut bank</li>
                </>
              )}
              {kind === 'exams' && (
                <>
                  ...
                  <li>subject_code harus ada jika diisi</li>
                </>
              )}
              {kind === 'grades' && (
                <>
