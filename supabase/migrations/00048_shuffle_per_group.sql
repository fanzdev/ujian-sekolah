-- Migration 00048: Shuffle soal dalam kelompok tipe yang sama
-- Objective (PG, PGK, B/S, Menjodohkan, Isian) diacak secara independen,
-- kemudian ditampilkan berurutan, baru disusul essay yang juga diacak sendiri.
...
begin
    ...
    begin
      ...
      if p_ip is not null and p_ip <> '' then
        ...
        if v_old_ip is not null and v_old_ip <> '' and v_old_ip <> p_ip then
          perform public.record_security_event(v_existing.id, 'IP_CHANGE', 'MEDIUM', jsonb_build_object('previous_ip', v_old_ip, 'new_device', p_device_info));
        end if;
      end if;
  ...
  -- Pisahkan ID soal berdasarkan tipe
  select array_agg(qid) into v_obj_ids
  from unnest(v_qids) as qid
  join public.exam_questions eq on eq.question_id = qid
  join public.questions q on q.id = eq.question_id
  where eq.exam_id = p_exam_id
    and q.type in ('multiple_choice', 'multiple_response', 'true_false', 'matching', 'short_answer');

  select array_agg(qid) into v_essay_ids
  from unnest(v_qids) as qid
  join public.exam_questions eq on eq.question_id = qid
  join public.questions q on q.id = eq.question_id
  where eq.exam_id = p_exam_id
  ...
  -- Acak setiap kelompok secara independen
  if v_exam.shuffle_questions then
    v_obj_shuffled := private.shuffle(coalesce(v_obj_ids, '{}'::uuid[]));
    v_essay_shuffled := private.shuffle(coalesce(v_essay_ids, '{}'::uuid[]));
    v_all_shuffled := v_obj_shuffled || v_essay_shuffled;
  else
    v_all_shuffled := v_qids;
  end if;

  -- Acak opsi dan pasangan untuk setiap soal
  for rec in
    select q.id, q.type
  ...
  insert into public.exam_attempts (
    ...
    to_jsonb(v_all_shuffled), v_opt_ord, v_match_ord,
    coalesce(
...
comment on function public.start_attempt(uuid, text, text, text, jsonb) is
  'Migration 00048: Shuffle terpisah per kelompok tipe (objective dulu, essay berikutnya)';
